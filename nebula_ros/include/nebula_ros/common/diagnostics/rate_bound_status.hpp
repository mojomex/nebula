...
// Copied and modified from https://github.com/tier4/ros2_v4l2_camera/pull/29
...
private:
  ...
  struct StateBase
  {
    StateBase(const unsigned char lv, const std::string m) : level(lv), num_observation(1), msg(m)
    {
    }
...
public:
  ...
  void run(diagnostic_updater::DiagnosticStatusWrapper & stat) override
  {
    ...
    if (candidate_state_.index() == frame_result.index()) {
      ...
      std::visit([](auto & s) { s.num_observation += 1; }, candidate_state_);
    } else {
