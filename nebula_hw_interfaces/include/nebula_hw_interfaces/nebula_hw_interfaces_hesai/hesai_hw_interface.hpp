...
#include "nebula_hw_interfaces/nebula_hw_interfaces_hesai/udp.hpp"
...
private:
  ...
  std::function<void(std::vector<uint8_t> & buffer, size_t)>
...
public:
  ...
  void ReceiveSensorPacketCallback(std::vector<uint8_t> & buffer, size_t n_dropped);
  ...
  /// @return Resulting status
  Status RegisterScanCallback(std::function<void(std::vector<uint8_t> &, size_t)> scan_callback);
